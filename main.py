...
from model.scarnet import Model
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        if args.train:
            current_model_path = save_model(args, epoch, p1, model, 'model')

            if args.refine:
                current_refine_path = save_model(args, epoch, p1, model_refine, 'refine')

            if p1 < best_mpjpe:
                best_epoch = epoch
                best_mpjpe = p1
                print(f"New best model found at epoch {best_epoch}, MPJPE: {best_mpjpe:.4f}")
