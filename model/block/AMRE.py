...
class AMRE_Branch_Full(nn.Module):
    def __init__(self, in_features, num_heads):
        ...
        self.branch_3x3 = nn.Sequential(
            DepthwiseConv1d(self.split_size, kernel_size=3),
        )
        self.branch_5x5 = nn.Sequential(
            DepthwiseConv1d(self.split_size, kernel_size=5),
        )
        self.branch_7x7 = nn.Sequential(
            DepthwiseConv1d(self.split_size, kernel_size=7),
        )
        self.branch_9x9 = nn.Sequential(
            DepthwiseConv1d(self.split_size, kernel_size=9),
        )
...
class AMRE_Full(BaseModule):
    def __init__(self, 
        ...
        self.amre_branch_full = AMRE_Branch_Full(
    ...
    def forward(self, x):
        ...
        x_mona = self.project1(x) 

        x_mona = x_mona.permute(0, 2, 1) 

        x_mona = self.amre_branch_full(x_mona) 

        x_mona = x_mona.permute(0, 2, 1) 
        x_mona = F.gelu(x_mona)
        x_mona = self.dropout(x_mona)
        x_mona = self.project2(x_mona)    

        output = identity + x_mona
        return output
